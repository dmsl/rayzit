...
public class BytesRefWritable implements WritableComparable<BytesRefWritable> {
  ...
  public void copyContent(BytesRefWritable obj) throws IOException{
    this.start = 0;
    this.length = obj.length;
    if(length > bytes.length) bytes = new byte[length + 10];
    System.arraycopy(obj.getData(), start, bytes, 0, length);
  }
}