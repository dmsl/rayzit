...
public class GreedyGrouping {
	...
	public GreedyGrouping(Configuration conf) throws IOException {
		this.conf = conf;
	...
	public void doGouping() throws IOException {
		...
		short[] initSetOfParts = initPartsForGroups(restOfPivots, toBeShuffled);
		short pid, gid, id;
	...
	/**
	 * return number of objects lying in array2 but not in array1
	 * @param array1
	 * @param array2
	 * @return
	 */
	private int getNumOfDiffObjects(Vector<Short> array1, Vector<Short> array2) {
		int numOfDiffObjects = 0;
		int i = 0, j = 0;
		short pid1, pid2;
		...
		while (j < array2.size()) {
			...
			if (pid2 == pid1) {
				...
				numOfDiffObjects += numOfObjectsInPivot.get(pid2);
				j ++;
			} else {
				i ++;
			}
	...
	private short[] initPartsForGroups(Vector<Short> pids, Vector<Short>[] toBeShuffled) throws IOException {
		short i, j, id;
		short[] initSetOfParts = new short[numOfGroups];
		Vector<Integer> numOfDiffObjects = new Vector<Integer>(numOfPivots);
		for (i = 0; i < numOfPivots; i++)
			pids.add(i);

		/** select the first pid */
		Random rand = new Random();
		id = (short) rand.nextInt(pids.size());
		initSetOfParts[0] = pids.get(id);

		/** compute the distance of pivots to pid */
		for (i = 0; i < numOfPivots; i++) {
			if (id == i) {
				numOfDiffObjects.add(0);
			} else {
				numOfDiffObjects.add(getNumOfDiffObjects(toBeShuffled[id], toBeShuffled[i]));
			}
		}
		/** remove the id */
		pids.remove(id);
		numOfDiffObjects.remove(id);
		for (i = 1; i < numOfGroups; i++) {
			id = selectNextPId(numOfDiffObjects);
			short tmpPId = pids.get(id);
			initSetOfParts[i] = tmpPId;
			/** remove the id */
			pids.remove(id);
			numOfDiffObjects.remove(id);
			// update the distance of the unselected pivots
			for (j = 0; j < pids.size() && i < numOfGroups - 1; j++) {
				numOfDiffObjects.set(j, numOfDiffObjects.get(i) + getNumOfDiffObjects(toBeShuffled[tmpPId], toBeShuffled[pids.get(j)]));
			}
		}
	...
	private short selectNextPId(Vector<Integer> numOfDiffObjects)
			throws IOException {
		short maxOfID = 0;
		float num, maxNum = 0;

		for (short i = 0; i < numOfDiffObjects.size(); i++) {
			num = numOfDiffObjects.get(i);
			if (maxNum < num) {
				maxNum = num;
				maxOfID = i;
			}
	...
	public static void main(String[] args) {
		...
		try {
			GreedyGrouping grouping = new GreedyGrouping(conf);
			grouping.doGouping();
