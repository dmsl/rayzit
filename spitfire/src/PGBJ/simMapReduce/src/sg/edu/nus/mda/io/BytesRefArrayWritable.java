 ...
 * BytesRefWritable.
 * 
 * even this is copy from Hive's BytesRefArrayWritable, it's slight changed 
 * so that BytesRefWritable[] clearly show the sequences
 */
...
public class BytesRefArrayWritable implements WritableComparable<BytesRefArrayWritable> {
  ...
  public BytesRefWritable[] getAll() {
  ...
  public int compareTo(BytesRefArrayWritable other) {
    ...
    if (sizeDiff != 0)
      throw new IllegalArgumentException("Number of columns is not the same.");
    int flag;
    for (int i = 0; i < valid; i++) {
      flag = bytesRefWritables[i].compareTo(other.get(i));
   ...
   */
  public int compareTo(BytesRefArrayWritable other, int[] colID){
    int flag;
    for (int i = 0; i < colID.length; i++) {
      flag = bytesRefWritables[colID[i]].compareTo(other.get(colID[i]));
      if(flag != 0)return flag;
    }
    return 0;
  }
  ...
  public void copyContent(BytesRefArrayWritable obj) throws IOException{
    if(valid != obj.size()){
      throw new IOException("the number of columns are different");
    }
    for(int i = 0;i < valid;i ++){
      this.bytesRefWritables[i].copyContent(obj.get(i));
    }
  }
  ...
  public String toString(){
    ...
    buf.append(valid + ": ");
    for(int i = 0;i < valid;i ++){
      buf.append(this.bytesRefWritables[i].toString()).append(",");
    }
    return buf.toString();
  }
